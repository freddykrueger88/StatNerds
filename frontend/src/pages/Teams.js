...
import { getTeamList, getTeamDetail } from '../services/api';
        ...
        <div style={{ flex: 1 }}>
          ...
          {(team.descriptionDE || team.descriptionEN) && (
            <p style={{ fontSize: '0.82rem', color: '#777', lineHeight: '1.6', margin: '0 0 0.8rem 0' }}>
              {(team.descriptionDE || team.descriptionEN)?.slice(0, 400)}
              {(team.descriptionDE || team.descriptionEN)?.length > 400 ? '...' : ''}
            </p>
          )}
...
export default function Teams({ theme }) {
  ...
  // Vereinsliste laden
  const listFetch = useFetch(() => getTeamList());
  const teamList  = Array.isArray(listFetch.data) ? listFetch.data : [];

  // Detail laden wenn ein Verein ausgewählt ist
  const detailFetch = useFetch(
    () => selected ? getTeamDetail(selected.id) : Promise.resolve(null),
    null,
    [selected?.id]
  );
  ...
  // Detail-Ansicht
  if (selected) {
    if (detailFetch.loading) return <p style={{ color: '#666', textAlign: 'center', marginTop: '3rem' }}>⏳ Lade Vereinsinfos...</p>;
    if (detailFetch.error)   return <ErrorState message={detailFetch.error} onRetry={detailFetch.refetch} icon='🏟️' />;
    if (detailFetch.data)    return <TeamDetail team={detailFetch.data} theme={theme} onBack={() => setSelected(null)} />;
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: '0.75rem' }}>
        {filtered.map(t => <TeamCard key={t.id} team={t} theme={theme} onClick={setSelected} />)}
      </div>
    </div>
  );
}
