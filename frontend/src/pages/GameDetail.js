...
function Timeline({ events }) {
  ...
  return (
    <div style={{ marginTop: '1rem' }}>
      ...
      {events.map((e, i) => {
        const isHome = e.team?.id === events[0]?.team?.id;
...
function H2HSection({ team1, team2, theme }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/games/bl1/h2h?team1=${encodeURIComponent(team1)}&team2=${encodeURIComponent(team2)}`)
      .then(r => r.json())
      .then(d => { setH2h(Array.isArray(d) ? d : []); setLoading(false); })
      .catch(() => setLoading(false));
  }, [team1, team2]);

  if (loading) return <p style={{ color: '#555', fontSize: '0.8rem' }}>⏳ Lade H2H...</p>;
  if (!h2h.length) return <p style={{ color: '#444', fontSize: '0.8rem' }}>Keine H2H-Daten verfügbar.</p>;
...
export default function GameDetail({ game, theme, onBack }) {
  ...
  // API-Football Stats laden wenn Key vorhanden
  useEffect(() => {
    if (!apiKey || !game.externalFixtureId) return;
    setLoadingStats(true);
    fetch(`/api/apifootball/stats/${game.externalFixtureId}`, {
  ...
  return (
    <div>
      <button onClick={onBack} style={{
        ...
        border: `1px solid ${theme.primary}`, borderRadius: '6px',
        padding: '0.3rem 0.8rem', cursor: 'pointer', marginBottom: '1rem', fontSize: '0.85rem'
      }}>← Zurück</button>
      ...
      <div style={{ ...block, background: 'linear-gradient(135deg,#1a1a2e,#16213e)', textAlign: 'center', padding: '2rem 1rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '2rem' }}>
          ...
          <div>
            ...
            {!isFinished && <div style={{ fontSize: '0.75rem', color: theme.primary, textAlign: 'center' }}>Anpfiff</div>}
          </div>
          <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ marginTop: '1rem', fontSize: '0.78rem', color: '#666', lineHeight: '1.8' }}>
            {game.goals.map((g, i) => (
              <span key={i} style={{ marginRight: '1rem' }}>
                ⚽ {g.goalGetterName} {g.matchMinute}'{g.isPenalty ? ' [P]' : ''}{g.isOwnGoal ? ' [ET]' : ''}
              </span>
            ))}
          </div>
        )}
