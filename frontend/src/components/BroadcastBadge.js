...
export default function BroadcastBadge({ matchDate, compact = false }) {
  const [country] = useLocalStorage('sn_country', 'DE');

  const { data } = useFetch(
    () => getBroadcast(matchDate, country),
    null,
    [matchDate, country]
  );
  ...
  const badges = data.broadcasters.map((b, i) => {
  ...
  if (compact) {
    return (
      <div style={{ display: 'flex', gap: '0.25rem', flexWrap: 'wrap', marginTop: '0.3rem' }}>
        {badges.map(({ b, i, style }) => (
          <span key={i} style={{ background: style.bg, color: style.color, fontSize: '0.6rem', fontWeight: 'bold', padding: '1px 5px', borderRadius: '3px' }}>
            {b.broadcaster}
          </span>
        ))}
      </div>
  ...
  return (
    <div style={{ marginTop: '0.8rem' }}>
      ...
      <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
        {badges.map(({ b, i, style }) => (
          <div key={i} style={{ background: style.bg, color: style.color, borderRadius: '5px', padding: '0.3rem 0.6rem', fontSize: '0.75rem', fontWeight: 'bold' }}>
            {b.broadcaster}
